...
function Logo() {
  return (
    <a href={BASE} className="obs-footer-logo d-flex align-items-center gap-2 text-decoration-none mb-3">
      <div className="obs-logo-mark">OBS</div>
      <span className="obs-footer-logo-text">Oregon Bike Shop</span>
    </a>
  )
}
...
const footerSections = [
  {
    title: 'Shop',
    links: [
      { label: 'Shop Bikes', href: `${BASE}shop` },
    ],
  },
  {
    title: 'Services',
                ...
                <Col key={section.title} xs={12} sm={6} md={3}>
                  ...
                  <ul className="list-unstyled obs-footer-links">
                    {section.links.map(link => (
                      <li key={link.href}>
                        <a href={link.href} className="obs-footer-link">{link.label}</a>
                      </li>
                    ))}
                  </ul>
                </Col>
